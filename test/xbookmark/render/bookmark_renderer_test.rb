...
describe Xbookmark::Render::BookmarkRenderer do
  ...
  it "renders a markdown file with stable frontmatter and section ordering" do
    ...
    assert_equal ["ozempic", "novo-nordisk"], front["topics"]
    ...
    assert_includes md, "[[topics/ozempic|ozempic]]"
    assert_includes md, "[[entities/novo-nordisk|novo-nordisk]]"
    ...
    # Section ordering
    idx = ->(s) { md.index(s) }
    assert_operator idx.call("## Author"), :<, idx.call("## Topics")
    assert_operator idx.call("## Topics"), :<, idx.call("## Entities")
    assert_operator idx.call("## Entities"), :<, idx.call("## Media")
  ...
  it "writes the markdown to <bookmark-wiki>/bookmarks/YYYY/MM/DD/<id>.md" do
    Dir.mktmpdir do |dir|
      ...
      assert_equal File.join(dir, "bookmarks", "2026", "01", "01", "1001.md"), path
  ...
  it "falls back to created_at for invalid bookmarked_at and renders captions plus unavailable quotes" do
    ...
    assert_equal "/vault/bookmarks/2026/02/03/1001.md", renderer.markdown_path_for(fallback_bookmark)
