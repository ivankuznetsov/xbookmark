...
RSpec.describe "v1 acceptance" do
  ...
  it "writes 10 markdown files, aux pages, no leftover .tmp, and qmd find returns hits" do
    ...
    fake_link_fetcher = double(:lf)
    allow(fake_link_fetcher).to receive(:fetch).and_return(
    ...
    expect(report.synced).to eq(10)
    expect(report.failed).to eq(0)
    expect(report.permanent_errors).to eq(0)
    ...
    expect(File.exist?(File.join(vault, "authors", "bob.md"))).to be(true)
    expect(File.exist?(File.join(vault, "topics", "ozempic.md"))).to be(true)
    expect(File.exist?(File.join(vault, "topics", "ai-agents.md"))).to be(true)
    expect(File.exist?(File.join(vault, "entities", "novo-nordisk.md"))).to be(true)
    expect(File.exist?(File.join(vault, "entities", "openai.md"))).to be(true)

    # Mode advanced to test_backfilled
    expect(store.mode).to eq("test_backfilled")
