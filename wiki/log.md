...
## [2026-06-15T22:30:00Z] offline note-source re-enrichment

**Action:** Added `xbookmark reenrich [--limit N]` to re-run the current enrichment contract over existing notes without re-fetching from X (branch `feat/offline-note-reenrich`).
...
## [2026-06-17T00:00:00Z] browser bookmark source (no dev API)

**Action:** Added an opt-in `browser` bookmark source that ingests a user's own X bookmarks via a real Ferrum-driven Chromium reading X's internal GraphQL, coexisting with the API source (branch `we-need-to-add-an-260616-094b`, units U1–U6).
**Pages updated:** wiki/browser-source.md (new), wiki/index.md, wiki/decisions.md, wiki/dependencies.md, wiki/log.md, README.md, .env.example, packaging/{deb/fpm.sh,aur/PKGBUILD.template}
**Decision:** Parity is achieved at the envelope boundary: `Browser::Normalizer` converts X's GraphQL `Bookmarks`/tweet responses into the exact API v2 envelope `X::Expansions` already consumes, so the pipeline/renderer/media/whisper/enrich/QMD code is unchanged. `Sync::Runner` was generalized from a single `x_client:` to an ordered `sources:` list (back-compat shim kept) selected by `XBOOKMARK_SOURCE` (api|browser|both) via `Sources::Factory`; in `browser` mode the `X_*` keys are optional. A dedicated isolated Chromium profile under `~/.config/xbookmark/browser-profile` is used; headed `auth login --browser` (one-time consent in the store meta table) logs in once, headless thereafter. System Chromium is required but never bundled (detected by `Browser::Chromium`; reported by `doctor`). On a scheduled run, a `Browser::SessionExpired` sets `report.session_expired`, fires `Notify.send`, and exits non-zero even under `--from-scheduler`, while a co-configured API source still syncs. The live browser→X path is validated by a local manual acceptance run; deterministic units (normalizer via committed real-shape fixtures, config gating, factory, notify, expiry, consent, Chromium detection) keep the 100% coverage gate green.
**Source:** `lib/xbookmark/browser/{chromium,session,errors,normalizer,graphql_capture,source,login}.rb`, `lib/xbookmark/sources/factory.rb`, `lib/xbookmark/{config,notify}.rb`, `lib/xbookmark/sync/{runner,report}.rb`, `lib/xbookmark/cli/{auth,sync,doctor}.rb`, `lib/xbookmark/paths.rb`, `xbookmark.gemspec`, and their tests + `test/fixtures/browser/`.

## [2026-06-18T00:00:00Z] browser source review-fix pass (6-review pass 1)

**Action:** Hardened the browser bookmark source against its hostile-surface and unattended-failure modes from the code review (branch `we-need-to-add-an-260616-094b`).
**Pages updated:** wiki/browser-source.md, wiki/log.md, README.md (Scheduling exit-divergence + `SESSION_EXPIRED` token)
**Decision:** `Sync::Runner` now isolates *any* per-source failure — auth/rate-limit/transient, a missing-Chromium `ConfigError`, and the `:resync` path — via a shared `SOURCE_BLOCK_ERRORS` set so one source can never abort a multi-source run; `get_tweet_any` falls through `SourceUnavailable` to the next source. `Report` collapsed `session_expired`/`expired_source` into a single nullable `expired_source` with a derived `session_expired?` (no illegal state), and `api_pages` was renamed to the source-neutral `source_pages`. `Browser::Normalizer` filters every GraphQL array to Hashes + guards a non-Hash root, rescues `TypeError` in `iso8601`, and emits millisecond ISO8601 for true API-v2 parity; `Browser::Source` wraps each page normalization, conforms `get_tweet` to raise `SourceUnavailable`/`TransientError` (never nil), reuses one Chromium across calls (`#close` owned by the Runner), bounds the walk (iteration cap + settle/empty-round distinction) and raises `SessionExpired` when zero authentic Bookmarks responses are captured. `GraphqlCapture` records `@seen` only after a successful parse and tracks capture failures. `Session` persists an on-disk profile (`incognito: false`), locks the profile + config dir to `0700`, and sets explicit Ferrum timeouts; the systemd unit gains `RuntimeMaxSec`. `Notify` was renamed `deliver` (no longer shadows `Object#send`), spawns detached, and escapes backslashes-before-quotes. `auth status` exits non-zero for a missing browser-only session; `auth login --browser --accept-risk` + a TTY guard keep consent from hanging unattended; `doctor` no longer nags browser-only users about API login.
**Source:** `lib/xbookmark/browser/{normalizer,graphql_capture,source,session,login}.rb`, `lib/xbookmark/sync/{runner,report}.rb`, `lib/xbookmark/sources/factory.rb`, `lib/xbookmark/cli/{auth,sync,doctor}.rb`, `lib/xbookmark/notify.rb`, `lib/xbookmark/scheduler/systemd.rb`, and their tests.

## [2026-06-18T00:00:00Z] browser source review-fix pass (6-review pass 3)

**Action:** Applied the accepted code-review findings for the browser bookmark source (branch `we-need-to-add-an-260616-094b`, fix pass 3).
**Pages updated:** wiki/commands.md, wiki/browser-source.md, wiki/log.md, README.md (browser stderr token table)
**Decision:** `Browser::Source` now re-runs `guard_session!(page)` after the timeline walk's empty-rounds break, so a session that expires *mid-walk* (X redirects to login and stops serving Bookmarks) is reclassified as `SessionExpired` instead of sealing a truncated backfill as complete. `Sync::Runner` rejects an empty source list at construction (an empty run would otherwise mark the store backfilled with zero fetches) and dropped the dead `respond_to?(:get_tweet)` guard in `get_tweet_any` (`get_tweet` is a mandatory `Sources::Factory` contract method). A new `Browser::ChromiumMissing < ConfigError` lets `auth login --browser` emit `CHROMIUM_MISSING` only for a genuinely missing browser and a distinct `CONFIG_ERROR` for other config errors (e.g. an invalid `XBOOKMARK_SOURCE` parsed by `load_offline`). The `doctor` help text now names the browser source, the README documents the full browser stderr-token contract, and the wiki corrects `report.expired_source` (read-only) to `report.mark_session_expired`. Tests cover each path; `rake coverage` stays at 100%.
**Source:** `lib/xbookmark/browser/{source,errors,session}.rb`, `lib/xbookmark/sync/runner.rb`, `lib/xbookmark/cli/{auth,..}.rb`, `lib/xbookmark/cli.rb`, and their tests.

## [2026-06-18T00:00:00Z] browser source review-fix pass (6-review pass 4)

**Action:** Applied the accepted code-review findings for the browser bookmark source (branch `we-need-to-add-an-260616-094b`, fix pass 4).
**Pages updated:** wiki/commands.md, wiki/log.md
**Decision:** `Browser::Source#get_tweet` now re-runs `guard_session!(page)` after the page settles and the capture drains empty, before raising the permanent `SourceUnavailable` — the same session-guard class as the pass-3 `walk_timeline` fix. A session that expires *after* `go_to` (X completes a login/checkpoint redirect while the page settles) would otherwise capture no tweet, settle cleanly, and be sealed as an unavailable tweet, letting a retry/resync mark the row gone instead of firing `SESSION_EXPIRED`; the re-guard reclassifies it as `SessionExpired` (re-login). Corrected the stale `wiki/commands.md` command-flow line: `backfill`/`sync`/`resync` build sources through `Xbookmark::Sources::Factory` (X API client, browser source, or both per `config.source`; browser-only mode builds no API client), not "create an X API client".
**Source:** `lib/xbookmark/browser/source.rb`, `wiki/commands.md`, and their tests.

## [2026-06-18T00:00:00Z] browser source review-fix pass (6-review pass 8)

**Action:** Applied the accepted code-review findings for the browser bookmark source (branch `we-need-to-add-an-260616-094b`, fix pass 8).
**Pages updated:** wiki/gaps.md, wiki/log.md
**Decision:** Recorded the deliberate "no CI coverage for the live browser→X path" gap in `wiki/gaps.md` so the `[[gaps]]` backlink from `wiki/browser-source.md` resolves; CLAUDE.md requires open questions/known gaps be tracked there. Threaded the `State::Store` into `Browser::Source` (via `Sources::Factory`) so the unattended sync/backfill path now fails fast with a `ConfigError` until the one-time ToS/account-risk consent marker (`browser_consent_at`, recorded by `Browser::Login`) exists — previously browser-mode sync could hit X before consent. Also made `PathBuilder#bookmark_date` fall back to an epoch sentinel when both timestamps are unparseable so a dropped `created_at` renders into a date shard instead of being silently dropped forever as a permanent error.
**Source:** `lib/xbookmark/browser/source.rb`, `lib/xbookmark/sources/factory.rb`, `lib/xbookmark/render/path_builder.rb`, `wiki/gaps.md`, and their tests.
