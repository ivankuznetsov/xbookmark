...
Taxonomy rebuilds are local data migrations. `xbookmark taxonomy rebuild --apply` writes a pre-apply snapshot and manifest under `.xbookmark`, renames numeric source notes, migrates real numeric thread pages to readable `thread-<id>` pages, prunes generated numeric singleton thread pages, materializes concept pages from SQLite concept metadata, updates `bookmarks.markdown_path` and generated page rows through path-only store methods, writes a graph-health report, and reindexes QMD. Rebuilds are forward-only: snapshots are manual recovery/audit evidence, while `partial_failure` leaves completed file repairs visible. Dry-run/audit reports use explicit states: `clean`, `proposed_changes`, `blocked_conflicts`, `applied`, and `partial_failure`.
