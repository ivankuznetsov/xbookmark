...
- `xbookmark taxonomy rebuild [--apply]` performs an offline taxonomy repair workflow. Without `--apply`, it is a dry-run and reports proposed changes. With `--apply`, it snapshots generated wiki directories for manual recovery/audit evidence, writes a manifest and graph-health report under `.xbookmark`, renames numeric source notes, migrates real numeric thread pages to readable `thread-<id>` pages, prunes generated numeric singleton thread pages, updates state paths, and reindexes QMD. Rebuilds are forward-only: completed repairs remain in place if a later operation reports `partial_failure`.
