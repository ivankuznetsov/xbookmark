...
- Rendering: `Xbookmark::Render::BookmarkRenderer` writes readable per-bookmark source notes with a required tweet ID suffix; `ConceptPage` and `ConceptIndex` maintain canonical concept graph pages with explicit source-note post lists; `AuxPage` still maintains authors and real thread pages, and refreshes legacy topic/entity pages with post lists while old notes reference them. Author aux summaries remain opt-in through `XBOOKMARK_AUX_SUMMARIES`.
