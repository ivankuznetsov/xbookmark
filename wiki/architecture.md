...
- Credential storage: `Xbookmark::Keystore` chooses macOS Keychain, Linux libsecret, or an env-file fallback. Linux libsecret is considered usable only when `secret-tool` is present and `DBUS_SESSION_BUS_ADDRESS` is non-empty; `Xbookmark::Keystore::Resolver` mirrors that gate before routed provider lookups so D-Bus-less hosts get the actionable keychain-unavailable hint. `Xbookmark::Keystore::AuthConfig` reads/writes provider backend routing in `~/.config/xbookmark/auth.toml` without storing secret values, validates TOML provider sections against `Xbookmark::Keystore::Provider::NAME_PATTERN`, and `Resolver` applies the CI/env, auth-routing, keychain/1Password, and env-fallback priority chain. CI mode is an exact `CI=true` or `XBOOKMARK_KEYS_FROM_ENV=1` environment short-circuit, not a fall-through probe.
