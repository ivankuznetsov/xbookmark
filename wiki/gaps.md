...
| Browser source CI coverage | The live browser→X path (AC1–AC4) cannot run in CI | By design there is no provisionable X account for CI and X's anti-bot defenses make a live login non-deterministic, so the real browser→X flow is validated only by a local manual acceptance run. The deterministic units (normalizer via committed real-shape fixtures, source selection, finish_walk classification, config/consent gating, notify, expiry signaling, Chromium detection) carry full automated coverage to keep the 100% gate green. See [[browser-source]]. |
...
- 2026-05-22: README command/config docs were reconciled to the implemented CLI so new setups do not follow unsupported `schedule`, `--config`, `auth logout`, `enrich`, or extra backfill/find flags. `auth refresh` is now implemented and documented.
