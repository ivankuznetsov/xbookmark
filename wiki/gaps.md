...
| AuthConfig public wiring | New auth routing storage is present, but no public CLI was found | Commit `57ced9b` adds `Xbookmark::Keystore::AuthConfig`, tests, and `tomlrb`; this refresh found no public command or README contract for selecting/removing auth routing, so user-facing routing behavior should not be documented as available yet. |
...
- 2026-05-30: Dependency coverage now records the `tomlrb` runtime dependency, `AuthConfig`'s `auth.toml` artifact, and credential-store external tools; public auth-routing CLI behavior remains a gap until implementation/docs land.
